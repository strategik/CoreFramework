    ...
    public class ConnectorAzureTests
    {
        ...
        public static void ClassInit(TestContext context)
        {
            ...
            if (!String.IsNullOrEmpty(TestCommon.AzureStorageKey))
            {
                ...
                // Create or overwrite the "myblob" blob with contents from a local file.
                using (var fileStream = File.OpenRead(@".\resources\custom.spcolor"))
                {
                    blockBlob.UploadFromStream(fileStream);
                }
                ...
                // Create or overwrite the "myblob" blob with contents from a local file.
                using (var fileStream = File.OpenRead(@".\resources\templates\ProvisioningTemplate-2015-03-Sample-01.xml"))
                {
                    blockBlob.UploadFromStream(fileStream);
                }
            }
        ...
        #endregion Azure connector tests
    }
}