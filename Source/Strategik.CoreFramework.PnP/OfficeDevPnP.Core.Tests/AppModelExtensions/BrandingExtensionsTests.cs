...
using Resources = OfficeDevPnP.Core.Tests.Properties.Resources;
...
namespace OfficeDevPnP.Core.Tests.AppModelExtensions
{
    ...
    public class BrandingExtensionsTests
    {
        ...
        private const string CAML_QUERY_FIND_BY_FILENAME = @"
                <View>
                    <Query>
                        <Where>
                            <Eq>
        ...
        private bool deactivateSiteFeatureOnTeardown = false;
        private bool deactivateWebFeatureOnTeardown = false;
        ...
        public void Initialize()
        {
            ...
            File.WriteAllBytes(customColorFilePath, Resources.custom);
            customBackgroundFilePath = Path.Combine(Path.GetTempPath(), "custombg.jpg");
            Resources.custombg.Save(customBackgroundFilePath);
        ...
        public void CleanUp()
        {
            ...
            if (File.Exists(customColorFilePath))
            {
                File.Delete(customColorFilePath);
            }
            if (File.Exists(customBackgroundFilePath))
            {
                File.Delete(customBackgroundFilePath);
            }
