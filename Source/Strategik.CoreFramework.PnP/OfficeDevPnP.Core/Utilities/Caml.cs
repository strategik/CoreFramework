...
namespace OfficeDevPnP.Core.Utilities
{
    ...
    {
        private const string VIEW_XML_WRAPPER = "<View><Query>{0}{1}</Query><RowLimit>{2}</RowLimit></View>";
        private const string FIELD_VALUE = "<FieldRef Name='{0}' {1}/><Value Type='{2}'>{3}</Value>";
        private const string FIELD_VALUE_ID = "<FieldRef ID='{0}' {1} /><Value Type='{2}'>{3}</Value>";
        private const string WHERE_CLAUSE = "<Where>{0}</Where>";
        private const string GENERIC_CLAUSE = "<{0}>{1}</{0}>";
        private const string CONDITION_CLAUSE = "<{0}>{1}{2}</{0}>";
        ...
        private static string Comparison(CamlComparisions comparison, string fieldValue)
        {
            return string.Format(GENERIC_CLAUSE, comparison, fieldValue);
        }

        private static string Condition(CamlConditions condition, string clause1, params string[] comparisonClauses)
        {
        ...
        private enum CamlConditions { And, Or }

        private enum CamlClauses { Where, OrderBy, GroupBy }
    }

    public class OrderByField
    {
        private const string ORDERBY_FIELD = "<FieldRef Name='{0}' Ascending='{1}' />";
